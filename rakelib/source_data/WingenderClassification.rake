namespace 'source_data' do
  ...
  file 'source_data/human_uniprot.txt.gz' do
    sh 'wget', 'http://www.uniprot.org/uniprot/?sort=score&desc=&compress=yes&query=organism:%22Homo%20sapiens%20(Human)%20[9606]%22&fil=&format=tab&force=yes&columns=id,entry%20name,genes(PREFERRED),genes(ALTERNATIVE),protein%20names', '-O', 'source_data/human_uniprot.txt.gz'
  end
