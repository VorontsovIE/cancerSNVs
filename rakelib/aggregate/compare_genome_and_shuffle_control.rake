  ...
  [:protected, :subjected].each do |protected_or_subjected|
    ...
    ['disruption', 'emergence', 'substitution-in-core'].each do |characteristic|
      ...
      Configuration.getAlexandrovWholeGenomeCancers.each do |cancer_type|
        folder = File.join(LocalPaths::Results, 'motif_statistics/common/', 'Alexandrov', cancer_type.to_s, 'any', protected_or_subjected.to_s, characteristic.to_s)
      ...
      output_filename = File.join(LocalPaths::Results, 'motif_statistics/shuffle_vs_genome_control', "#{protected_or_subjected}_#{prep}_#{characteristic}_in_any_context.tsv")
