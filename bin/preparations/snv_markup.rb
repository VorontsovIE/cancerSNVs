...
puts SNVInfo::HEADER
SNVInfo.each_in_file(snvs_filename).each do |snv|
  chromosome = snv.chromosome
  snv.mutation_region_types.discard_types! # discard all mutation types

  snv.mutation_region_types << :promoter  if is_promoter.(chromosome, snv.position)
  snv.mutation_region_types << :intronic  if is_intronic.(chromosome, snv.position)
  snv.mutation_region_types << :kataegis  if is_kataegis.(chromosome, snv.position)
