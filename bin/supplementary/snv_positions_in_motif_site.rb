...
mutation_profiles = sites_filenames.map do |sites_filename|
  ...
  # PerfectosAPE::ResultShort.each_in_file(sites_filename).select{|site| site.disrupted?(fold_change_cutoff: 1) }.each do |site|
  PerfectosAPE::ResultShort.each_in_file(sites_filename).each do |site|
