...
sample_names = Dir.glob(File.join(LocalPaths::Results, 'motif_statistics/full/samples/*')).select{|fn|
  File.directory?(fn) 
}.map{|fn|
  File.basename(fn)
...
sample_infos = sample_names.map{|sample_name|
  rate_infos_by_sample = random_variants.flat_map{|random_variant|
    filename = File.join(LocalPaths::Results, 'motif_statistics/full/samples/', sample_name, "#{random_variant}.csv")
