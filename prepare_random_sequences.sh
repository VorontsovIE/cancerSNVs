...
for SEED in ${RANDOM_GENOME_SEEDS}; do
  ...
  ruby bin/preparations/generate_random_genome_sequences.rb  ${SNV_FOLDER}/SNV_infos_cancer.txt  --fold 100  --random-seed $SEED --flank-length 50  >  ${SEQ_FOLDER}/sequences_random_genome_${SEED}.txt
  ...
  ruby bin/preparations/snv_markup.rb ${SNV_FOLDER}/SNV_infos_random_genome_${SEED}_non_marked_up.txt  >  ${SNV_FOLDER}/SNV_infos_random_genome_${SEED}.txt
