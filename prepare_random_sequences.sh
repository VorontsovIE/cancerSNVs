...
for SEED in ${RANDOM_GENOME_SEEDS}; do
  ...
  ruby bin/preparations/generate_random_genome_sequences.rb  ${SNV_FOLDER}/SNV_infos_cancer.txt  ./source_data/exons.txt  ./source_data/genome  --fold 10  --random-seed $SEED --flank-length 50  >  ${SEQ_FOLDER}/sequences_random_genome_${SEED}.txt
